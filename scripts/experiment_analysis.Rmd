...
Prior, a weakly informative prior on the slope coefficient with SD = 2:
...
E1_coefs <- posts_long %>% group_by(Coefficient) %>% 
...
E2_coefs <- posts_long %>% group_by(Coefficient) %>% 
  summarize(M = mean(Estimate),
            LowerCI = quantile(Estimate, 0.025),
            UpperCI = quantile(Estimate, 0.975))
```
...
ggsave(plot = both_coef_p, filename = '../data/both_coefficient_plots.pdf',
       width = 12, height = 6)
```
